...
public class TodoListWindow extends BasicWindow  {
    ...
    private void initWindow() {
        ...
        addButton = new Button("Dodaj", () -> {
            ...
            if (task.isEmpty() || task.length() > 255)
                return;

            updateSize();

            try {
                dateTime = convertToDate(dateTextBox.getText(), dateTimeFormat);
        ...
        panel.addComponent(new Label("Enter - zatwierdź; Del - usuń; F2 - edytuj; Kl. strzałek - nawigacja"));
        setComponent(panel);
        ...
        todoTable.setPreferredSize(new TerminalSize(80, 9));
    }

    /**
     * Dodaje nowe zadanie do listy z bieżącą datą i czasem jako czasem utworzenia.
     *
     * @param task Opis zadania do dodania.
     */
    public void addItemToList(String task) {
    ...
    /**
     * Dodaje nowe zadanie do listy z określonym czasem utworzenia.
     *
     * @param task Opis zadania do dodania.
     * @param dateTime Data i czas wygaśnięcia zadania.
     */
    public void addItemToList(String task, LocalDateTime dateTime) {
        if (task.isEmpty() || dateTime == null)
            return;
